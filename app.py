...
from flask import Flask, render_template, request, redirect, url_for, session
# ... (بقية الاستيرادات)
from database import get_db_connection, init_db, seed_db # مهم!
import sqlite3
...
# =======================================================
# ✅ الحل النهائي: تنفيذ التهيئة مباشرة
# =======================================================
try:
    print("--- 🛠️ بدء تهيئة قاعدة البيانات على الخادم ---")
    # هذه الدوال ستُنفذ مرة واحدة عند بدء تشغيل Gunicorn
    init_db() 
    seed_db()
    print("--- 🟢 تم إعداد قاعدة بيانات SQLite3 بالبيانات الأولية ---")
except Exception as e:
    # هذا للتصحيح في سجلات Render
    print(f"ERROR: فشل في تهيئة قاعدة البيانات: {e}")
# =======================================================

# --- 2. دالة مساعدة: دمج المقاطع... (بقية الكود)
# ...
